...
export class MotiValidators {

  static atLeastOneNotEmpty() {
    return (controlGroup: FormGroup) => {
      const valid = MotiValidators.checkFormGroup(controlGroup, field => field.value);
      if (!valid) {
        return {
          atLeastOneNotEmpty : {
  ...
  static atLeastOneNotValid() {
    return (controlGroup: FormGroup) => {
      const valid = MotiValidators.checkFormGroup(controlGroup, field => field.valid);
      if (!valid) {
        return {
          atLeastOneNotEmpty : {
            text : 'At least one value should be set'
    ...
    if (controls) {
      return Object.keys(controls).some(key => {
        ...
        if (field instanceof FormGroup || field instanceof FormArray) {
          return MotiValidators.checkFormGroup(field, operation);
        }
